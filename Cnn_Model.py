...
class TextCNN(object):
    def __init__(self, config):
        ...
        filter_sizes = config.filter_sizes  #句子的断句的filter
        ...
        for filter_size in filter_sizes:
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                ...
                #tf.Variable（inital_value（初始化变量值）,name）
                W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W") #产生正态分布，shape是获取的句子，stddev为标准差
                #常量张量
        ...
        self.h_pooled=tf.concat(pooled_outputs, 3) #concat是连接两个矩阵，pooled_outputs的维度
        ...
        with tf.name_scope("output"):
            ...
            self.result=tf.matmul(self.h_drop,self.Weight)+self.bias #matmul相当于矩阵相乘
            self.predictions=tf.argmax(self.result,1,name="predictions") #对矩阵按行或列计算最大值
